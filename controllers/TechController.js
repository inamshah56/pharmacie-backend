    ...
    try {
      upload(req, res, async function (err) {
        if (err instanceof multer.MulterError) {
          ...
          const fieldsToLowerCase = ['name', 'brand', 'description', 'areaCovered'];
          ...
          if (!check) return apiResponse.ErrorResponse(res, "Product already exists");
          ...
          } else if (req.body.productType == "Machinery & Tools") {
            console.log("Machinery & Tools");
            ...
            if (req.body.composition && req.body.composition.length > 0) {
              for await (let comp of req.body.composition) {
                await Models.Composition.create({
                  ...
                  unit: comp.unit.toLowerCase(),
                  volume: comp.volume.toLowerCase(),
          ...
          return apiResponse.successResponse(res, "Product Stored Successfully");
        }
      });
    } catch (err) {
      console.log(err);
